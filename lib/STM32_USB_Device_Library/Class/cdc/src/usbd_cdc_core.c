 ...
 *********************************************/
extern void Handle_USBAsynchXfer  (void *pdev);
...
//uint8_t __attribute__((aligned(2)) USB_Rx_Buffer[CDC_DATA_MAX_PACKET_SIZE] ;
 __align(2) uint8_t USB_Rx_Buffer[CDC_DATA_MAX_PACKET_SIZE];

static uint8_t CmdBuff[CDC_CMD_PACKET_SZE] ;
...
static uint8_t  usbd_cdc_DataIn (void *pdev, uint8_t epnum)
{
	APP_FOPS.pIf_DataTx(pdev, NULL, 0);
...
{      
  ...
  APP_FOPS.pIf_DataRx(pdev, USB_Rx_Buffer, USB_Rx_Cnt);
